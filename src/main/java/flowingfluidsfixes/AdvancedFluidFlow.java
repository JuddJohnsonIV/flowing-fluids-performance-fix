...
public class AdvancedFluidFlow {
    private static final Logger LOGGER = LoggerFactory.getLogger(AdvancedFluidFlow.class);
    ...
    public static void simulateFluidFlow(Level level, BlockPos pos, BlockState state) {
        ...
        // Debug logging to reference variables
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Processing fluid at level {}, position {}, with data {}", level, pos, data);
        }
    ...
    private static void calculateFiniteFluidPhysics(Level level, BlockPos pos, FluidFlowData data) {
        ...
        LOGGER.debug("Calculated finite fluid physics for level {}, position {}, pressure {}, flow direction {}, flow rate {}", level, pos, data.pressure, data.flowDirection, data.flowRate);
    }
    ...
    private static void applyFiniteFlowUpdates(Level level, BlockPos pos, FluidFlowData data) {
        ...
        LOGGER.debug("Applied finite flow updates for level {}, position {}, flow direction {}, flow rate {}", level, pos, data.flowDirection, data.flowRate);
    }
    ...
    private static void applyFiniteSpreadingFlow(ServerLevel level, BlockPos pos, FluidFlowData data) {
        ...
        LOGGER.debug("Applied finite spreading flow for level {}, position {}", level, pos);
    }
    ...
    private static void transferFiniteFluid(ServerLevel level, BlockPos from, BlockPos to, FluidFlowData data) {
        int transferAmount = Math.min(data.fluidState.getAmount() / 2, 1); // Transfer half or minimum 1 unit
        if (transferAmount > 0) {
            transferFiniteFluidAmount(level, from, to, transferAmount);
        }
        
        LOGGER.debug("Transferred finite fluid from level {}, position {} to position {}", level, from, to);
    }
    ...
    private static void transferFiniteFluidAmount(ServerLevel level, BlockPos from, BlockPos to, double amount) {
        ...
        LOGGER.debug("Transferred finite fluid amount from level {}, position {} to position {}", level, from, to);
    }
    ...
    private static void markActiveFlowRegion(Level level, BlockPos pos) {
        ...
        LOGGER.debug("Marked region as active for level {}, position {}", level, pos);
    }
    ...
    public static void cleanupFlowData(Level level) {
        ...
        LOGGER.debug("Cleaned up old flow data for level {}", level);
    }
